...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting SuperAI Pro Custom Credits flow for:', { name, email });
    ...
    console.log('SuperAI Pro flow: Test clock created:', testClock.id);
    ...
    console.log('SuperAI Pro flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/SuperAI/customunitssuccess?session_id={CHECKOUT_SESSION_ID}`,
      ...
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your SuperAI Pro Plan subscription.'
        }
      }
    });

    console.log('SuperAI Pro flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in SuperAI Pro Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create SuperAI Pro Custom Credits flow',
