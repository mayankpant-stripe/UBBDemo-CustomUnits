...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting SuperAI Enterprise Custom Credits flow for:', { name, email });
    ...
    console.log('SuperAI Enterprise flow: Test clock created:', testClock.id);
    ...
    console.log('SuperAI Enterprise flow: Customer created:', customer.id);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        plan: 'superai_enterprise_plan',
        pricing_plan_id: 'bpp_test_61TeLpk36MZRGgydI16T5kls95SQJJF9DR1pbaQwq7js',
      ...
      custom_text: {
        submit: {
          message: 'Complete payment setup to activate your SuperAI Enterprise Plan subscription.'
        }
      }
    });

    console.log('SuperAI Enterprise flow: Checkout session created:', { id: session.id, customerId: customer.id, testClockId: testClock.id });
    ...
    console.error('Error in SuperAI Enterprise Custom Credits flow:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to create SuperAI Enterprise Custom Credits flow',
